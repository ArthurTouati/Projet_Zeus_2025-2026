...
class RocketData:
    def __init__(self):
        # Header :
        self.rocket_header = ['Temp sol (HH:MM:SS)', 'Temp fusee (s)', 'Position x (m)', 'Position y (m)', 'Position z (m)', 'Acceleration x (m/s2)', 'Acceleration y (m/s2)', 'Acceleration z (m/s2)', 'Gyroscope x (rad/s)', 'Gyroscope y (rad/s)', 'Gyroscope z (rad/s)', 'Magnetometre x (uT)', 'Magnetometre y (uT)', 'Magnetometre z (uT)', 'Quaternion i', 'Quaternion j', 'Quaternion k', 'Quaternion reel', 'Roulis (deg)', 'Tangage (deg)', 'Lacet (deg)']
        self.time_header = ['gt','rt']
        self.gps_header = ['x','y','z']
        self.imu_header = ['ax','ay','az','gx','gy','gz','mx','my','mz','qi','qj','qk','qreal','roll','pitch','yaw']
        ...
        self.gps_dict = dict.fromkeys(self.gps_header)
        self.imu_dict = dict.fromkeys(self.imu_header)
        self.trajectory_dict = dict.fromkeys(self.rocket_header)
    ...
    def convert_data_into_trajectory(self):
        ...
        j = 0
        for i in self.time_header:
            self.trajectory_dict[self.rocket_header[j]] = self.time_dict[i]
            j += 1

        # Convert GPS dictionary :
        for i in self.gps_header:
            self.trajectory_dict[self.rocket_header[j]] = self.gps_dict[i]
            j += 1

        # Convert IMU dictionary :
        for i in self.imu_header:
            self.trajectory_dict[self.rocket_header[j]] = self.imu_dict[i]
            j += 1
